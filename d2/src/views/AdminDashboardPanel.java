    ...
    implements NumpadListener
{
    ...
    private void adminDashboard() {
        ...
        listModel.addRow(new Object[] {
            i18n.get("label.selectTransaction", "Please select a function."),
        });
        listModel.addRow(new Object[] {
            i18n.get("label.useButtons", "Use the buttons on the sides."),
        });
    ...
    private void adminRateSelect() {
        ...
        for (Map.Entry<String, Double> e : ExchangeRateManager.getInstance()
            .getConfiguredRates()
            .entrySet())
            listModel.addRow(new Object[] {
                e.getKey(),
        ...
        if (listTable.getRowCount() > 0) listTable.setRowSelectionInterval(
            0,
            0
        );
    ...
    private void adminRateInput(String pair) {
        ...
        centerTitle.setText(
            i18n.get("label.newRate", "New rate") + ": " + pair
        );
        listModel.setColumnIdentifiers(new String[] { "", "" });
        listModel.setRowCount(0);
        ...
        listModel.addRow(new Object[] {
            ...
            String.format(
                "%.4f",
                ExchangeRateManager.getInstance().getRate(parts[0], parts[1])
            ),
        });
        centerText.setText(
            i18n.get("label.enterRate", "Enter new rate: ") + "0"
        );
        buf.setLength(0);
        buf.append("0");
        dotEntered = false;
    ...
    private void adminUnlock() {
        ...
        centerText.setText(
            i18n.get("label.selectUserPrompt", "Select a user, then press OK")
        );
    ...
    private void setupArrowButtons() {
        ...
        rightBtns[3].addActionListener(e -> {
            ...
            if (
                r < listTable.getRowCount() - 1
            ) listTable.setRowSelectionInterval(r + 1, r + 1);
        });
    }
    ...
    public void onNumber(int n) {
        if (subState.equals("rate_input")) {
            if (
                dotEntered &&
                buf.indexOf(".") >= 0 &&
                buf.length() - buf.indexOf(".") > 4
            ) return;

            if (buf.toString().equals("0") && n != 0) {
                buf.setLength(0);
                buf.append(n);
            } else if (!buf.toString().equals("0")) buf.append(n);
            centerText.setText(
                i18n.get("label.enterRate", "Enter new rate: ") + buf.toString()
            );
        }
    }
        ...
        if (subState.equals("rate_select")) {
            ...
            try {
                ...
                if (r <= 0) {
                    centerText.setText(
                        "<html><font color=red>" +
                            i18n.get("error.positiveRate", "Must be > 0") +
                            "</font></html>"
                    );
                ...
                ExchangeRateManager.getInstance().setRate(
                    parts[0],
                    parts[1],
                    r
                );
                ...
                Timer t = new Timer(1500, ev -> adminRateSelect());
                t.setRepeats(false);
                t.start();
            } catch (NumberFormatException ex) {
                centerText.setText(
                    "<html><font color=red>" +
                        i18n.get("error.invalidRate", "Invalid") +
                        "</font></html>"
                );
            }
            ...
            if (r < 0) {
                centerText.setText(
                    i18n.get("error.selectUser", "Select a user")
                );
                return;
            }
            ...
            if (u.isLocked()) {
                ...
                centerText.setText(
                    "<html><font color=green>" +
                        i18n.get("info.userUnlocked", "Unlocked") +
                        ": " +
                        u.getUserName() +
                        "</font></html>"
                );
                ...
                centerText.setText(
                    i18n.get("info.userNotLocked", "Already active")
                );
            }

            Timer t = new Timer(1500, ev -> adminUnlock());
            t.setRepeats(false);
            t.start();
        }
    }
    ...
    public void onCancel() {
        if (subState.equals("rate_input")) {
            ...
            subState.equals("rate_select") || subState.equals("unlock")
        ) {
            adminDashboard();
        }
    }
